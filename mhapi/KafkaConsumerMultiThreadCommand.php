...
class KafkaConsumerMultiThreadCommand extends Command
{
    ...
    public function handle()
    {
        ...
        $params = array();
        $exampleTask = new Example(['boo'=>'whoo']);
        $multithreadManager = new Multiple(['boo'=>'whoo']);
...
//                   $this->findEmails($offset, $limit);
//                   echo "Child $status completed\n";
//               }
    }

    protected function findEmails($offset, $limit)
    {
        dd($offset.' / '.$limit);
        $Email = GroupEmailModel::where('status','=','pending-sending')->take($limit)->get();
                if(!empty($Email[0]))
                {
                    $this->sendByPHPMailer($Email);
                }
    }
