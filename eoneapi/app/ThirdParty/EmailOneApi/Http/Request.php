<?php
/**
 * This file contains the EmailOneApi_Http_Request class used in the EmailOneApi PHP-SDK.
 * 
 *
 *
 *
...
/**
 * EmailOneApi_Http_Request is the request class used to send the requests to the API endpoints.
 * 
 *
 * @package EmailOneApi
 * @subpackage Http
 * @since 1.0
 */
class EmailOneApi_Http_Request extends EmailOneApi_Base
{
    /**
     * @var EmailOneApi_Http_Client the http client injected.
     */
    public $client;
    
    /**
     * @var EmailOneApi_Params the request params.
     */
    public $params;
     ...
     * Constructor.
     * 
     * @param EmailOneApi_Http_Client $client
     */
    public function __construct(EmailOneApi_Http_Client $client)
    {
        $this->client = $client;    
    }
    ...
    public function send()
    {
        ...
        curl_setopt($ch, CURLOPT_USERAGENT , 'EmailOneApi Client version '. EmailOneApi_Http_Client::CLIENT_VERSION);
        ...
        if ($client->isPostMethod || $client->isPutMethod || $client->isDeleteMethod) {
            ...
            $params = new EmailOneApi_Params($client->paramsPost);
        ...
        $params = $this->params = new EmailOneApi_Params($curlInfo);
        ...
        if ($curlCode === 0 && $client->getResponseHeaders) {
            ...
            $params->add('headers', new EmailOneApi_Params($headers));
        }
        
        $decodedBody = array();
        if ($curlCode === 0 && !empty($body)) {
            $decodedBody = EmailOneApi_Json::decode($body, true);
        ...
        $params->add('body', new EmailOneApi_Params($decodedBody));
        
        $response = new EmailOneApi_Http_Response($this);
        $body = $response->body;
    ...
    protected function sign($requestUrl)
    {
        ...
        $params = new EmailOneApi_Params($specialHeaderParams);
