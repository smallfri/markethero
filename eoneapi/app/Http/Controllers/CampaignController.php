...
END;
        ...
        $response = $this->endpoint->create(array(
            ...
            'options' => array(
                'url_tracking'      => $data['url_tracking'], // yes | no
                'json_feed'         => $data['json_feed'], // yes | no
                'xml_feed'          => $data['xml_feed'], // yes | no
                'plain_text_email'  => $data['plain_text_email'],// yes | no
                'email_stats'       => $data['email_stats_address'], // a valid email address where we should send the stats after campaign done
                $autoresponder
